#  This script is running only for lowercase and uppercase letters !! There is no script only for uppercase letters !
#  Check others scripts for other implementations
...
def check_length_psw():
    ...
    while True:
        guess_pass = random.choices(string.ascii_letters, k=length_of_searched_psw)
        guess_concat = ''.join(guess_pass)
        # if len(found_pass.keys()) == len(string.ascii_letters) ** length_of_searched_psw:
        #     return 0, 0, False
